...
doVlnPlot <- function(aspect, seur, thresh=c(), group='Sample') {
  temp_plot <- VlnPlot(seur, group.by=group, features=aspect, pt.size=0) + geom_boxplot(width=0.2, position=position_dodge(0.9), outlier.size=1) + NoLegend()
...
Idents(seur) <- 'Sample'
plots <- sapply(c("nFeature_RNA", "nCount_RNA", "percent.mito"), function(x) doVlnPlot(aspect=x, seur=seur))
