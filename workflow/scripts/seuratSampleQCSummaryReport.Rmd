...
#cbind(sample_list, lapply(sample_list, function(x) file.path(maindir, x, plt3) %>% pander::pandoc.image.return()), lapply(sample_list, function(x) file.path(maindir, x, plt1) %>% pander::pandoc.image.return())) %>% as.data.frame %>% pander

#data <- cbind(sample_list, lapply(sample_list, function(x) paste0(file.path(maindir, x, plt3) %>% pander::pandoc.image.return(), '{height=260px, width=260px}')), lapply(sample_list, function(x) paste0(file.path(maindir, x, plt1) %>% pander::pandoc.image.return(), '{height=260px, width=520px}')), lapply(sample_list, function(x) paste0(file.path(maindir, x, plt5) %>% pander::pandoc.image.return(), '{height=260px, width=260px}'))) %>% as.data.frame()
#colnames(data) <- c('Sample', 'Violin Plots', 'Scatter Plots', 'UMAP Plots')
data <- cbind(sample_list, lapply(sample_list, function(x) paste0(file.path(maindir, x, plt3) %>% pander::pandoc.image.return(), '{height=260px, width=260px}')), lapply(sample_list, function(x) paste0(file.path(maindir, x, plt1) %>% pander::pandoc.image.return(), '{height=260px, width=520px}'))) %>% as.data.frame()
...
```{r  echo=F ,results='asis'}
data <- cbind(sample_list, lapply(sample_list, function(x) paste0(file.path(maindir, x, plt4) %>% pander::pandoc.image.return(), '{height=260px, width=260px}')), lapply(sample_list, function(x) paste0(file.path(maindir, x, plt2) %>% pander::pandoc.image.return(), '{height=260px, width=520px}'))) %>% as.data.frame()
colnames(data) <- c('Sample', 'Violin Plots', 'Scatter Plots')
data %>% kable(format='html') %>%
        column_spec(column=length(data) %>% seq(), border_left = "1px solid gray", border_right = "1px solid gray") %>%
        row_spec(row=dim(data)[[1]] %>% seq(), hline_after=T, align='c', extra_css = "border-bottom: 1px solid gray") %>%
...
```{r  echo=F ,results='asis'}
data <- cbind(sample_list, lapply(sample_list, function(x) paste0(file.path(maindir, x, plt9) %>% pander::pandoc.image.return(), '{height=512px, width=576px}'))) %>% as.data.frame()
