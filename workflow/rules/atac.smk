...
pipeline_output += expand(
    join(workpath, "{sample}", "outs", "web_summary.html"), sample=samples
)
...
pipeline_output += expand(
    join(workpath, "cleanup", "{sample}.samplecleanup"), sample=samples
)
...
def filterFastq(wildcards):
    ...
    for sample in sample_rename(wildcards).split(","):
        filter_paths += [
            ...
            if len(re.findall(f"{sample}_[\w]*R2[\w]*.fastq.gz", i)) > 0
        ]
