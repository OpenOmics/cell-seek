# <code>cell-seek <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>cell-seek <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking cell-seek pipeline output directory is fast and easy! In its most basic form, <code>cell-seek <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./cell-seek unlock [-h] --output OUTPUT
```
...
cell-seek unlock --output /data/$USER/output
```